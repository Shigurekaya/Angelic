...
LOG = OUT_DOC / "full-static-rest.log"
REPORT = OUT_DOC / "full-static-rest-report.json"

# 剩余：立绘/CG/adult/upgrade。排除 voice。
ARCHIVES = [
    "fgimage.xp3",
    "evimage.xp3",
    "adult.xp3",
...
def wait_done(name, before, timeout=1800, stable=60):
...
def organize_ui():
    ...
    copied = 0
    summary = {}
    for src in sorted(EXTRACT_OUT.iterdir()):
        if not src.is_dir():
            continue
        stem = src.name
        dest = FULL_OUT / stem
    ...
    return {"copied": copied, "summary": summary, "out": str(FULL_OUT)}


def main():
    OUT_DOC.mkdir(parents=True, exist_ok=True)
    log("==== Angelic REST static unpack start ====")
    log("scope=%s (fg/ev/adult/upgrade)" % ",".join(ARCHIVES))
    log("exclude=voice.xp3; no scn/bgm packs")
    append_md(
        "\n### %s — 启动剩余包 Cxdec 解包\n\n"
    ...
    for name in ARCHIVES:
        ...
        if not p.exists():
            log("missing %s" % name)
            continue
        stem = name.replace(".xp3", "")
        done_dir = EXTRACT_OUT / stem
        if done_dir.exists() and count_files(done_dir) > 0:
            log("skip already done %s files=%s" % (name, count_files(done_dir)))
            results.append({"archive": name, "skipped": True, "files": count_files(done_dir)})
            continue
